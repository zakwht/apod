...
export const getLatest = (callback: (state: IAPODResponse[]) => void) => {
  ...
  sendRequest(`&start_date=${startQuery}&end_date=${endQuery}`, (req: XMLHttpRequest) => {
    const response: IAPODResponse[] = req.status === 200 ? JSON.parse(req.response) : samplePhotos[Math.floor(Math.random() * samplePhotos.length)];
    callback(response);
    localStorage.setItem("APODHistory", JSON.stringify({
...
export const getRandom = (callback: (state: IAPODResponse[]) => void) => {
  ...
  if (cached) {
    ...
    callback(response);
    if (response.length) return localStorage.setItem("APODRandom", JSON.stringify(response.slice(1)));
  }
